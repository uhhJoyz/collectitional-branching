...
def benchmark_matmul(device="cpu", M=512, K=512, N=512, trials=10):
    ...
    if not _fits_in_memory_elems(total_elems):
        return {"op": "matmul", "device": device, "shape": (M, K, N), "skipped": True,
    ...
    if device == "cpu":
        ...
        return {"op": "matmul", "device": "cpu", "shape": (M, K, N), "mean_ms": mean, "std_ms": std, "skipped": False}

    elif device == "gpu":
        device_torch = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
        A = torch.rand((M, K), dtype=torch.float32, device=device_torch)
        B = torch.rand((K, N), dtype=torch.float32, device=device_torch)
        ...
        return {"op": "matmul", "device": device_torch.type, "shape": (M, K, N), "mean_ms": mean, "std_ms": std, "skipped": False}

    else:
        raise ValueError("device must be 'cpu' or 'gpu' or 'mps'")

sizes = [2**i for i in range(6, 13)]  # 64 → 4096
cpu_times = []
gpu_times = []
...
for n in sizes:
    res_cpu = benchmark_matmul("cpu", n, n, n)
    res_gpu = benchmark_matmul("gpu", n, n, n)
...
plt.plot(sizes, cpu_times, "o-", label="CPU (PyTorch NumPy)")
...
plt.title("CPU vs GPU Matrix Multiplication Benchmark (PyTorch)")
plt.legend()
plt.grid(True, which="both", ls="--", alpha=0.5)
plt.tight_layout()

plt.savefig("../results/matmul_benchmark_torch.png")
...
df.to_csv("../results/matmul_results.csv", index=False)
